...
/**
 * Kid right-rail bundle (profile chip + activity + daily challenge + stats).
 * Composed as four widgets so each can also be used independently in the
 * generic right-rail render path.
 */
...
export function KidProfileChip() {
...
export function KidTodaysActivity() {
  const { creationsByType } = useAiPoints();

  const realInProgress = Object.entries(creationsByType ?? {})
    .filter(([, count]) => count > 0)
    .slice(0, 3)
    .map(([type, count]) => ({ type, count }));

  // If we have no real activity AND placeholders are on, show placeholder data
  // so the surface visualises what it'll look like once data lands.
  const placeholdered = realInProgress.length === 0 && isPlaceholder(PLACEHOLDERS.todaysActivity);
  const items = realInProgress.length > 0 ? realInProgress : PLACEHOLDERS.todaysActivity.value;

  return (
    <div className={cn('relative rounded-2xl bg-white p-4 shadow-card', placeholdered && 'opacity-80')}>
      {placeholdered && <PlaceholderRibbon />}
...
export function KidDailyChallenge() {
  const placeholdered = isPlaceholder(PLACEHOLDERS.dailyChallenge);
  return (
    <Link
      href="/beat-the-ai"
      className={cn(
        'group relative flex flex-col overflow-hidden rounded-2xl bg-gradient-to-br from-orange-400 via-pink-400 to-rose-400 p-4 text-white shadow-card transition hover:-translate-y-0.5 hover:shadow-elevated',
        placeholdered && 'opacity-90',
      )}
    >
      {placeholdered && <PlaceholderRibbon variant="onColor" />}
      ...
      <p className="mt-1 max-w-[60%] text-[11px] text-white/85">Win 30 AI Points</p>
...
export function KidStatsGrid() {
  const { activeKid } = useKidProfile();
  const { totalPoints, badges } = useAiPoints();
