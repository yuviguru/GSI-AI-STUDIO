...
- **Character consistency (reference conditioning)**: pure text-to-image (text anchor + seed) drifts across pages, so `book-generate` generates ONE clean hero **anchor portrait** (Flux txt2img) and **reference-edits it onto the cover + every page**. The image port carries a `referenceImageUrl` + two capability flags (`supportsText2Img`, `supportsReference`); `imageRouter` routes reference requests only to edit-capable providers via `requireReference` + `preferProviders` (cost-tier-bounded). The kid picks `quality` in the AI-generate form — **Standard** = Qwen-Image-Edit (`pixazo-qwen-edit`, same Pixazo key, tier `cheap`); **Premium** = Nano Banana (Gemini Flash Image) → gpt-image fallback (tier `premium`, key-gated). Premium gracefully degrades to Qwen / txt2img when no premium key is set. The anchor is persisted on the cast member (`BookCharacter.anchorImageUrl`) and reused by later per-page regeneration.
- **Async generation (BOOK-008)**: the reference pipeline (anchor + slow edit models) doesn't fit Netlify's 26s sync window, so `POST /api/ai/book-generate` now **bills + writes a `pending` shell (`createPendingGeneratedBook`) + enqueues + returns instantly** — superseding the old in-request image phase. `lib/books/enqueueBookGenerationJob` runs `lib/books/generateBookJob` **inline in dev** / via the **`netlify/functions/book-generate-background.mts`** background function (15-min budget, token-gated by `INTERNAL_FUNCTION_TOKEN`, URL `BOOK_GENERATE_BACKGROUND_URL`) in prod — mirroring the WhatsApp background pattern. The job drafts → writes pages → renders the anchor → reference-edits cover + pages (retry ≤3), streaming `generation.{status,step,pagesRendered}` that the home tile polls via SWR (`useBookList` refreshInterval); `finalizeBookGeneration` sets `complete`/`partial`/`failed` and **refunds** via `refundCredits` on a hard failure. `BookReadyWatcher` (global, in the public layout) shows an in-app toast on completion; a `failed` book retries via `POST /api/ai/book-generate/[id]/retry` using its stored `generationInput`.
