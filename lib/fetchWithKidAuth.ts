 ...
 *
 *  Kid CEO is authenticated-only, so AI Points + badges now write directly
 *  to `kids/{kidId}` via `updateKidPoints` — the session doc isn't involved.
 *  That means we intentionally do NOT forward `X-Session-Id` from this
 *  fetcher; the older dual-write fallback is gone.
 *
 ...
 *  token" (401) or "Pick a kid profile first" (400). */
